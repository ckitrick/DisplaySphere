...
//	static char				cdBuffer[512];
	...
	arg = _ds_add_option_choice_argument(opt, "which", sizeof(int), (void*)&ctx->curInputObj.fAttr.hole.style, 0, 
		"round", (void*)&fexd[0], "polygonal", (void*)&fexd[1], 0);// pairs of ("text", void *srcData);
...
//-----------------------------------------------------------------------------
{
	...
	fprintf(fp, "-face_hole_type %s\n", gobj->fAttr.hole.style ? "polygonal" : "round");
	fprintf(fp, "-face_hole_radius %f\n", gobj->fAttr.hole.radius);
	// scale
...
//-----------------------------------------------------------------------------
{
	...
	fprintf(fp, "-stereo_cross_eye %s\n", ctx->drawAdj.stereoCrossEyeFlag ? "disable" : "enable");
